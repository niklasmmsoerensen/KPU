// SimpelCalcClientV2.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if(SUCCEEDED(hr))
	{
		...
		//////////////////////////////////////////////////////////////////////////////
		// Test af Advanced version
		hr = CoCreateInstance( CLSID_Calculator, NULL, CLSCTX_INPROC_SERVER,
			IID_IAdvCal, (void**) &pIAdvMath);

		// If succeeded then call the Mul method, if it fails
		// then display an appropriate message to the user.
		if (SUCCEEDED(hr))
		{
			hr = pIAdvMath->Mul(3, 3, &ReturnValue);
			cout << "The answer for 3 * 3 is: " << ReturnValue << endl;
		}
		else ShowErrorMessage(hr);

		if (SUCCEEDED(hr))
		{
			hr = pIAdvMath->Div(12, 3, &ReturnValue);
			cout << "The answer for 12 / 3 is: " << ReturnValue << endl;
		}
		else ShowErrorMessage(hr);
